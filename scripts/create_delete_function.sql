-- Function to safely delete a user and all their associated data
-- This avoids needing to change all FOREIGN KEY constraints to CASCADE,
-- which might be brittle if the schema is updated via Supabase UI.

CREATE OR REPLACE FUNCTION public.delete_user_entirely(target_user_id UUID)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER -- Needs to run as superuser to delete from auth.users
AS $$
BEGIN
    -- 1. Delete interventions (references hives)
    DELETE FROM public.interventions WHERE hive_id IN (
        SELECT id FROM public.hives WHERE apiary_id IN (
            SELECT id FROM public.apiaries WHERE user_id = target_user_id
        )
    );

    -- 2. Delete inspections (references hives)
    DELETE FROM public.inspections WHERE hive_id IN (
        SELECT id FROM public.hives WHERE apiary_id IN (
            SELECT id FROM public.apiaries WHERE user_id = target_user_id
        )
    );

    -- 3. Delete hive_snapshots (references hives)
    DELETE FROM public.hive_snapshots WHERE hive_id IN (
        SELECT id FROM public.hives WHERE apiary_id IN (
            SELECT id FROM public.apiaries WHERE user_id = target_user_id
        )
    );

    -- 4. Delete tasks (references apiaries)
    DELETE FROM public.tasks WHERE apiary_id IN (
        SELECT id FROM public.apiaries WHERE user_id = target_user_id
    );
    
    -- 4b. Delete global tasks for this user (if any exist independently of apiaries)
    DELETE FROM public.tasks WHERE user_id = target_user_id;

    -- 5. Delete hives (references apiaries)
    DELETE FROM public.hives WHERE apiary_id IN (
        SELECT id FROM public.apiaries WHERE user_id = target_user_id
    );

    -- 6. Delete apiaries (references users)
    DELETE FROM public.apiaries WHERE user_id = target_user_id;
    
    -- 7. Delete feature_requests (references users)
    DELETE FROM public.feature_requests WHERE user_id = target_user_id;
    
    -- 8. Delete ai_qa_history (references users)
    DELETE FROM public.ai_qa_history WHERE user_id = target_user_id;
    
    -- 9. Delete user_roles
    DELETE FROM public.user_roles WHERE user_id = target_user_id;
    
    -- 10. Delete mentor relationships
    DELETE FROM public.mentor_students WHERE mentor_id = target_user_id OR student_id = target_user_id;

    -- 11. Delete from public.users
    DELETE FROM public.users WHERE id = target_user_id;

    -- 12. Finally, delete from auth.users
    DELETE FROM auth.users WHERE id = target_user_id;

    RETURN true;
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error deleting user %: %', target_user_id, SQLERRM;
        RETURN false;
END;
$$;
